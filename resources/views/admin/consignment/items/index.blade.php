...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        ...
        <button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#modalTambahBarang"
            ...
            <i class="fa-solid fa-plus me-1"></i> Terima Barang Konsinyasi
        </button>
    </div>
        ...
        <div class="card-body p-3">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    @if(request()->hasAny(['consignor_id','status','search']))
                        <a href="{{ route('admin.consignment.items.index') }}" class="btn btn-outline-secondary" style="border-radius: 10px;"><i class="fa-solid fa-xmark"></i></a>
                    @endif
                </div>
            </form>
        </div>
                        ...
                        <tr>
                            <td class="ps-4">
                                ...
                                <small class="text-muted">
                                    ...
                                    @if($item->expiry_date)
                                        · <span class="text-danger">Exp {{ $item->expiry_date?->format('d M Y') }}</span>
                                    @endif
                                </small>
                            </td>
                            <td>
                                @if($item->consignor?->trashed())
                                    <span class="text-muted fw-semibold">
                                        <i class="fa-solid fa-user-slash me-1"></i>{{ $item->consignor?->name }} (Terhapus)
                                    </span>
                                @else
                                    <a href="{{ route('admin.consignors.show', $item->consignor) }}" class="text-decoration-none fw-semibold" style="color: var(--color-primary-dark);">
                            ...
                            <td class="text-center">
                                ...
                                @if($item->commission_rate !== null)
                                    <br><small class="text-muted" style="font-size:.7rem;">override</small>
                                @endif
                            </td>
...
{{-- ============================================================ --}}
{{-- Modal Terima Barang Konsinyasi --}}
<div class="modal fade" id="modalTambahBarang" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <form action="{{ route('admin.consignment.items.store') }}" method="POST" class="modal-content" style="border-radius: 16px; border: none;">
            @csrf
            ...
            <div class="modal-body px-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="consignor_id" class="form-select" required style="border-radius: 10px;" id="consignorSelect">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-3 align-items-center mt-1">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="product_mode" id="modeNew" value="new" checked>
                                <label class="form-check-label" for="modeNew">Produk Baru</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="product_mode" id="modeExisting" value="existing">
                                <label class="form-check-label" for="modeExisting">Produk Yang Sudah Ada</label>
                            </div>
                        </div>
                    </div>

                    {{-- Form Produk Baru --}}
                    <div id="sectionNew" class="col-12">
                    ...
                    {{-- Form Produk Lama --}}
                    <div id="sectionExisting" class="col-12 d-none">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="commission_rate" id="commissionRateInput" class="form-control" min="0" max="100" step="0.5" placeholder="Pakai tarif penitip" style="border-radius: 10px 0 0 10px;">
...
<script>
// Toggle antara mode produk baru vs lama
document.querySelectorAll('[name="product_mode"]').forEach(r => {
    r.addEventListener('change', function () {
        document.getElementById('sectionNew').classList.toggle('d-none', this.value !== 'new');
        document.getElementById('sectionExisting').classList.toggle('d-none', this.value !== 'existing');
    });
});

// Auto-isi komisi dari tarif penitip yang dipilih
document.getElementById('consignorSelect').addEventListener('change', function () {
